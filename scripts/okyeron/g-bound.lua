...
function freqtomidinote(freq)
    midinote = (12/math.log(2)) * math.log(freq/27.5) + 21
    return midinote
end 
...
function play_notes()
  while #freq_queue > 0 do
    this_freq = table.remove(freq_queue)
    --print (freqtomidinote(this_freq))
    engine.hz(this_freq)
    clk_midi.note_on(freqtomidinote(this_freq),100)
    --clk_midi.note_off(freqtomidinote(this_freq))
  end
end
